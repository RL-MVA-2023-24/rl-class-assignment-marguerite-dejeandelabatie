...
class ProjectAgent:
    def __init__(self, config=config, model=None):
        ...
        if model is not None:
            ...
            self.optimizer = config['optimizer'] if 'optimizer' in config.keys() else torch.optim.Adam(self.model.parameters(), lr=self.learning_rate)
            

    def _aux_multiproc(self, env):
        x,_ = env.reset()
        done = False
        trunc = False
        total_reward = 0
        ...
        for _ in range(50):
            ...
            if done or trunc:
                break
        return np.array([discounted_reward, total_reward])
        

    def MC_eval(self, env, nb_trials):   # NEW NEW NEW
        # MC_total_reward = []
        # MC_discounted_reward = []
        # answer = pool_obj.map(self._aux_multiproc, range(nb_trials))
        result = Parallel(n_jobs=10)(delayed(self._aux_multiproc)(env) for _ in range(nb_trials))
        return np.mean(result, axis=0)
        # for _ in tqdm(range(nb_trials), disable=True):
    ...
    def V_initial_state(self, env, nb_trials):   # NEW NEW NEW
        with torch.no_grad():
            for _ in range(nb_trials):
                val = []
                x,_ = env.reset()
                val.append(self.model(torch.Tensor(x).unsqueeze(0).to("cpu")).max().item())
        return np.mean(val)
    ...
    def train(self, env, max_episode):
        ...
        for episode in range(max_episode):
            for _ in range(100):#20+int(episode*0.5)):
                state = next_state
                # update epsilon
...
if __name__ == "__main__":
    ...
    ep_length, disc_rewards, tot_rewards, V0 = agent.train(env, 50)
